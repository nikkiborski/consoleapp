...
func getFileInformationByHandleEx(handle syscall.Handle, data *fileBasicInfo) (err error) {
	...
	r1, _, e1 := syscall.SyscallN(procGetFileInformationByHandleEx.Addr(), 4, uintptr(handle), uintptr(fileBasicInfoClass), uintptr(unsafe.Pointer(data)), unsafe.Sizeof(*data), 0, 0)
